...
.factory('planService', function ($q, $http, $filter, Config) {
    ...
    planService.getTypedPlaces = function (i) {
        ...
        var names = [];
        i = i.replace(/\ /g, "+");
        var url = "https://os.smartcommunitylab.it/core.geocoder/spring/address?latlng=" + Config.getLat() + "," + Config.getLon() + "&distance=" + Config.getDistanceForAutocomplete() + "&address=" + i;
        $http.get(url, {
            timeout: 5000
        }).
